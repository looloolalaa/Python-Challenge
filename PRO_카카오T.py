# 카카오 2차 코딩 테스트
# x_auth_token 에 request 때려서 점수 획득
import requests
import json
import random
...
# 근처들 중에서 차이가 2 보다 큰 애한테 1 씩 옮기기
def simulate1():
    send = get_send()
    commands = []
    for s in send:
...
temp = [[1, 2], [2, 3], [3, 4], [4, 1]]  # random directions
pattern = 0
hot_places = [[3037, 3386], [2635], [2465]]
cool_places = [[3595], [628, 724], [969, 3515]]


# 맵이 60 * 60 으로 크기 때문에 꽤 다른 로직 필요
# 핫+쿨 플레이스에 하나씩 트럭 담당(3개)
# 남은 트럭들(7개)은 랜덤으로 움직이며 자전거 균등 분배
def simulate2():
    # print(trucks)
    global pattern
    commands = []
